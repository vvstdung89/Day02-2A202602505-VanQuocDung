...
| -------------------------------- | -------------------------------------------------------------------------------------- | --------------------------------------------- | ----------------------------------- | --------------------------------------------- | ---------------------------------------------------- |
| OpenAI Whisper                   | [https://github.com/openai/whisper](https://github.com/openai/whisper)                 | Phiên âm audio → text (ASR)                   | Độ chính xác cao, hỗ trợ tiếng Việt | Cần GPU hoặc API có phí; latency nếu file dài | Dùng làm bước 1 (transcribe) trong pipeline          |
| Otter.ai                         | [https://otter.ai](https://otter.ai)                                                   | Record + transcribe + highlight key moment    | UX tốt, real-time                   | Tiếng Việt chưa tốt; phí cao ở tier đầy đủ    | Benchmark để so sánh output chất lượng               |
| LLM Summarization (GPT-4/Gemini) | [https://platform.openai.com/docs/overview](https://platform.openai.com/docs/overview) | Tóm tắt văn bản dài → bullet points / Q&amp;A | Linh hoạt prompt, hiểu ngữ cảnh     | Hallucination nếu transcript nhiễu            | Dùng làm bước 2 (summarize); cần human review output |
...
Sử dụng model lớn để tăng độ chính xác.
```
...
| **Boundary** (làm / không làm)                                                     | LÀM: pipeline transcribe + summarize cho audio/video do nhóm tự thu (có đồng ý). KHÔNG LÀM: lưu trữ dài hạn, chấm điểm tự động, xử lý file bên thứ ba chưa xin phép.                         |
...
| **Mức chọn** (Rule / Workflow / Agent + 1 câu vì sao)                              | **Workflow** — các bước tuần tự, xác định, orchestrate được bằng code; không cần AI tự lập kế hoạch động (**agent**). **Rule base** không phù hợp khi giải quyết bài toán ngôn ngữ tự nhiên. |
